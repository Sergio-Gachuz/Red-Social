<?php
	...
	class LynxSpace{
                                    ...
                                    <form method="GET" action="index.php">
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-2">
                                                <input type="submit" href="index.php" name="respuesta" value="Enviar" class="btn btn-dark btn-block">
                                            </div>
                                        </div>
                                    </form>
                                </div>          
        ...
        function pulgares($id_mensaje){
	        ...
	        $sentencia->execute();
	        $fila=$sentencia->fetch();
	        $pulgar='           '.$fila['numero'].'  <a href="index.php?accion=reaccion&reaccion=1&id_mensaje='.$id_mensaje.'"><img src="image/like.png" width="25" height="25"></a>';
	        ...
	        if(isset($fila['id_persona'])){
                $pulgar.='      <a href="index.php?accion=reaccion&reaccion=0&id_mensaje='.$id_mensaje.'"><img src="image/angry.png" width="25" height="25"></a>';
            }
            return $pulgar;
        }
		...
		function sugerencias($id_persona){
            $this->conexion();
            $amigos=array();
            $sql = "SELECT p.id_persona,p.nombre, p.apellidos, u.email,p.foto,p.apodo,foobarfoo.amigos from (SELECT id_persona, count(id_persona) as 'amigos' from (SELECT id_persona from (SELECT id_persona from (SELECT id_persona from amistad where id_amigo in(
            SELECT id_persona FROM persona where id_persona in (SELECT id_amigo from  amistad where amistad.id_persona=:id_persona UNION ALL SELECT amistad.id_persona from  amistad where  id_amigo=:id_persona))
            UNION ALL
            SELECT id_amigo from amistad where id_persona in(
            SELECT id_persona FROM persona where id_persona in (SELECT id_amigo from  amistad where amistad.id_persona=:id_persona UNION ALL SELECT amistad.id_persona from  amistad where  id_amigo=:id_persona))) foo
            
                WHERE id_persona!=:id_persona) bar
                WHERE id_persona not in (SELECT id_amigo from  amistad where amistad.id_persona=:id_persona UNION ALL SELECT amistad.id_persona from  amistad where  id_amigo=:id_persona)) foobar
                group by 1
                order by 2 desc) foobarfoo
                JOIN persona p on foobarfoo.id_persona=p.id_persona
                JOIN usuario u on p.id_usuario = u.id_usuario
                ORDER BY  amigos desc limit 10;";
            $sentencia = $this->conexion->prepare($sql);
            $sentencia->bindParam(':id_persona', $id_persona);
            $sentencia->execute();
            $i=0;
        ...
        function correoAmistad($destino, $mensaje, $asunto, $nombre){
            ...
            $mail->SMTPAuth = true;
            $mail->Username = '';//de donde se manda
            $mail->Password = '';//contraseña
            $mail->setFrom('', 'Lynx-Space');//quien manda
        ...
        public function amigos_nuevos(){
            $this->conexion();
            $sql = 'SELECT DISTINCT(a.id_persona), p.* from persona p inner join amistad a on p.id_persona = a.id_persona where p.id_persona not in (select id_persona from amistad where id_amigo = :id) and p.id_persona not in (select id_amigo from amistad where id_persona=:id) and p.id_persona != :id';
            $sentencia = $this->conexion->prepare($sql);
            $sentencia->bindparam(':id', $_SESSION['id_persona']);
            if(!$sentencia->execute())
                print_r( $sentencia->errorInfo()[2]);
